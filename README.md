...
| ⇅ **GitHub** | Two-way GitHub sync for the open generator — **⬇ Pull** (repo → editor, undoable) and **⬆ Push** (editor → repo in one atomic commit), with per-generator file mappings and global repo defaults. |
...
- **Declared `@connect` hosts:**
  ...
  @connect api.github.com                       # GitHub Push (Git Data API, with your token)
...
- **GitHub Push writes both editor panes in one atomic commit.** Use **Diff** to review changes before pushing. If GitHub changes while a push is in flight, GitHub rejects the ref update and the branch is unchanged.
